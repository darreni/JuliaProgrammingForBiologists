...
The object `G` is of type `Matrix{Any}` which means that it is a matrix that can store anything in its small boxes. In contrast, the object `A` is of type `Matrix{Int64}` which means that it can only store integers. If we try to put something else in its boxes, we get an error:
...
As one example, let's say we need to store a big matrix of random values, lets say 10000 by 10000, so 100 million values. We could just venture forth with a simple command, like this:

```{julia}
bigMatrix = fill(1, 10_000, 10_000)    
```

This creates a 10000 by 10000 array and fills it with the `1`s. We can check the memory size of this array like this:
...
```{julia}
selectionMask = (m1 .% 3 .== 0)  # This indicates all the values that are divisible by 3
```
