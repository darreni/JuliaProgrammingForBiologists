...
The cells (or elements) within Arrays and DataFrames can be used to store all sorts of different kinds of data, ranging from simple (e.g. `Int`s, `Float`s, `Char`s) to complex (e.g. arrays, tuples, or our own types created using `struct`). To understand the behaviour of data structures containing these more complex element types, we should now learn some about _pointers_.