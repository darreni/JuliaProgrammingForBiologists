...
We envision a 2-dimensional plot representing the geographic range, with dots representing individuals at their locations. We can use 3 colors to represent the three strategies. Let's define the color key using a _dictionary_:

```{julia}
plot_colors = Dict('R'=>"red", 'P'=>"green2", 'S'=>"blue") 
```

Now let's write a function that uses the above data structures to plot the population:
