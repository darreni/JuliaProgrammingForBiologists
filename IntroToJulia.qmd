...
Below demonstrates another way to write functions, in this case with two _arguments_ (things you pass into the function):
...
***Try this:*** Can you figure out how to call the function in a way where you can input a series of numbers as the first argument (for example, 'x = [1, 3, 7]'), and '-2' as the second argument? (if you get an error, reviewing the Broadcasting section above might help)
...
First, enter the _package mode_ of the REPL by typing ']' . You will then see the 'julia>' prompt change into a prompt that looks something like '(@v1.10) pkg>' . Then, type a command to add a package, e.g.:
...
To get out of package mode, press 'delete' and this will return you to the normal Julia REPL mode.
