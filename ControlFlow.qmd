...
Note that the `$` in the `println` line above causes _string interpolation_ of the expression to the right of it. 
...
::: {.callout-tip}
## Teach Julia to Sing
Write a script that produces the song "99 bottles of beer on the wall." This song goes like "99 bottles of beer on the wall, 99 bottle of beer, you take one down and pass it around, 98 bottles of beer on the wall; 98 bottles of beer on the wall, 98 bottles of ..." and so forth all the way down until there is no more beer. (I don't think anyone has ever completed the song . . . Can your program succeed in doing so?)
:::
...
You can _iterate_ across all sorts of things, for example a matrix of strings:
...
```{julia}
mySet = ["banana", 5.3, '🐬']  # for the dolphin:  \:dolphin: tab
for thing ∈ mySet
...
```{julia}
startLocation = 0.7
newLocation = -1.0 # initialize at not-allowed value
while !(0.0 < newLocation < 1.0)
  newLocation = startLocation + randn()
end
println(newLocation)
```
:::
...
The above _returns_ the value of the last subexpression, and it does remember values assigned in the earlier subexpressions. (To prove this to yourself, enter `x` to see its value.)
...
_If you succeed: Can you now generate a 4×4×4 `Array` that contains all possible **tri**nucleotides?_ 
:::

::: {.content-hidden}
```{julia}
nucs = ['A', 'C', 'G', 'T']
dinucs = [x*y for x in nucs, y in nucs]
```

```{julia}
trinucs = [x*y*z for x in nucs, y in nucs, z in nucs]
```
:::
...
Now that we have have developed an understand of the key elements of programming, in the next pages let's learn some more about visualizing data and then build a fun simulation.
