...
In the above, the arguments given to `plot!()` are _anonymous functions_. This means that are not given names and are not stored in memory. Rather, the syntax `x -> f(x)` simply provides a mapping of x onto y values, and the `plot!()` function interprets that and shows us a part of the relationship.
...
```{julia}
xes = -1:0.01:1
p5 = surface(xes, xes, (x, y) -> x^2 + y^2)
```

Here, the first argument is the `x` values, the second is the `y` values, and the thir is the `z` values, which in this case we provided as an anonymous function relating `x` and `y` to `z`.
